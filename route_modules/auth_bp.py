...
@auth_bp.route('/api/auth/verify-status')
def api_verify_status():
    uid = session.get('user_id')
    if uid:
        u = User.query.get(uid)
        if u:
            return jsonify({'logged_in': True, 'legal': True, 'psycho': True, 'email': u.email or ''})
    return jsonify({
        'logged_in': False,
        'legal': bool(session.get('email_verified_for_legal')),
        'psycho': bool(session.get('email_verified_for_psycho')),
        'email': session.get('verify_email') or '',
    })
