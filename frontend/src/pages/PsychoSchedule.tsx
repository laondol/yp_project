...
import type { PsychoAppointment } from '../lib/types'
...
export default function PsychoSchedule() {
  ...
  const [myAppts, setMyAppts] = useState<PsychoAppointment[]>([])
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [s, a] = await Promise.all([
        psychoApi.schedules(),
        psychoApi.appointments().catch(() => []),
      ])
      setSchedules(s)
      setMyAppts(Array.isArray(a) ? a : [])
    } catch (e: unknown) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/psycho/appointment/book', { method: 'POST', body: fd }).then(r => r.json())
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
        ...
        <div className="card-body p-4">
          <form onSubmit={handleSubmit}>
            <div className="mb-3">
              ...
              <input type="text" className="form-control form-control-sm" placeholder="상담 제목" required />
            </div>
              ...
              <div className="row g-2">
                <div className="col-6">
                  <select className="form-select" value={location} onChange={e => setLocation(e.target.value)} required>
                    ...
                    <option value="심리상담소">심리상담소 방문</option>
            ...
            <div className="alert alert-warning py-2 small mb-3">상담 내용 및 장소에 따라 상담비와 출장비가 발생할 수 있습니다.</div>
      ...
      <div className="text-center mt-3">
        <button className="btn btn-sm btn-outline-secondary" onClick={() => navigate('/psycho')}>← 심리상담소</button>
      </div>
    </div>
  )
}
