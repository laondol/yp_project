...
export default function ServicePsychoEditPage() {
  ...
  if (!canManage) {
    return (
      <div className="alert alert-danger text-center py-4">
        ...
        <p>관리자만 접근할 수 있습니다.</p>
        <button className="btn btn-sm btn-outline-danger" onClick={() => navigate('/service/psycho')}>돌아가기</button>
      </div>
    )
  }
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/service/psycho/edit', {
        method: 'POST',
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
      ...
      <div className="d-flex gap-2">
        ...
        <button className="btn btn-outline-secondary px-4 py-2" onClick={() => navigate('/service/psycho')}>미리보기</button>
      </div>
    </div>
  )
}
